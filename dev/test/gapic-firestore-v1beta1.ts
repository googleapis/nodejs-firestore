...
describe('v1beta1.FirestoreClient', () => {
    ...
    describe('write', () => {
        ...
        it('invokes write with error', done => {
            ...
            const stream = client.write().on('data', () =>{
                assert.fail();
            }).on('error', (err: FakeError) => {
                assert(err instanceof FakeError);
                assert.strictEqual(err.code, FAKE_STATUS_CODE);
    ...
    describe('listen', () => {
        ...
        it('invokes listen with error', done => {
            ...
            const stream = client.listen().on('data', () =>{
                assert.fail();
            }).on('error', (err: FakeError) => {
                assert(err instanceof FakeError);
                assert.strictEqual(err.code, FAKE_STATUS_CODE);
    ...
    describe('listCollectionIds', () => {
        it('invokes listCollectionIds without error', done => {
            ...
            const expectedResponse = {};
            // Mock Grpc layer
            client._innerApiCalls.listCollectionIds = (actualRequest: {}, options: {}, callback: Callback) => {
                assert.deepStrictEqual(actualRequest, request);
                callback(null, expectedResponse);
            };
    ...
    describe('listCollectionIdsStream', () => {
        it('invokes listCollectionIdsStream without error', done => {
            ...
            const stream = client.listCollectionIdsStream(request, {}).on('data', (response: {}) =>{
                assert.deepStrictEqual(response, expectedResponse);
                done();
            }).on('error', (err: FakeError) => {
                done(err);
