...
import IFunctionExpression = google.firestore.v1.IFunctionExpression;
import IArrayValue = google.firestore.v1.IArrayValue;
...
export class FunctionOrder {
   ...
   */
  toProto(): api.StructuredQuery.IOrder {
    let funcExpr: IFunctionExpression | null = null;
      ...
      funcExpr = {
        ...
        argumentList: {
          expressions: [
            ...
            {constantExpression: {arrayValue: distanceFunc.targetVector()}},
...
> implements firestore.OnceQuery<AppModelType, DbModelType>
{
  ...
  orderBy(
    ...
    if (fieldPath instanceof FieldPath || typeof fieldPath === 'string') {
      ...
      newOrder = {
        type: 'function',
        order: new FunctionOrder(f, directionOperators[directionStr || 'asc']),
      };
    }
  ...
  private createImplicitOrderBy(
    ...
    const lastDirection =
      fieldOrders.length === 0
        ? directionOperators.ASC
        : fieldOrders[fieldOrders.length - 1].order.direction;
