    ...
    describe('arrayFilter', () => {
        ...
        const snapshot = await firestore
          ...
          .select(
            field('arr')
              ...
              .as('result1'),
            arrayFilter('arr2', 'element', field('element').greaterThan(3)).as(
              'result2',
            ),
            arrayFilter('empty', 'element', field('element').greaterThan(3)).as(
              'result3',
            ),
            arrayFilter(
              'nullArr',
      ...
      it('supports arrayFilter edge case missing property in map', async () => {
        const snapshot = await firestore
          ...
          .select(
            arrayFilter('arr', 'element', field('element.a').greaterThan(1)).as(
              'result',
            ),
          )
          .execute();
