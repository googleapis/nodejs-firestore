...
function getTestRoot(firestore) {
  return firestore.collection(
    `node_${version}_${Firestore.autoId(firestore)}`);
}
...
describe('CollectionReference class', function() {
  ...
  it('has add() method', function() {
    return randomCol.add({foo: 'a'}).then((ref) => {
      return ref.get();
    }).then((doc) => {
...
describe('DocumentReference class', function() {
  ...
  it('supports server timestamps', function() {
    ...
    return ref.set(baseObject).then(() => {
      return ref.get();
    }).then((doc) => {
  ...
  describe('watch', function() {
    ...
    it('handles changing a doc', function() {
      ...
      return waitForSnapshot().then((snapshot) => {
    ...
    it('handles multiple docs', function(done) {
      ...
      let run = [
        () => { doc1.set({foo: 'foo'}); doc2.set({foo: 'foo'}); },
        () => { doc1.delete(); doc2.delete(); },
        () => { unsubscribe1(); unsubscribe2(); done(); }
      ];
...
describe('Query class', function() {
  ...
  it('has select() method', function() {
    let ref = randomCol.doc('doc');
    return ref.set({foo: 'bar', bar: 'foo'}).then(() => {
  ...
  it('suports NaN and Null', function() {
    ...
    return ref.set({foo: NaN, bar: null}).then(() => {
      ...
      assert.ok(typeof res.docs[0].get('foo') === 'number' &&
  ...
  it('has stream() method', function(done) {
    ...
    Promise.all([
      ref1.set({foo: 'a'}),
      ref2.set({foo: 'b'})]).then(() => {
        return randomCol.stream().on('data', () => {
          ++received;
        }).on('end', () => {
  ...
  describe('watch', function() {
    ...
    it('handles changing a doc', function() {
      ...
      let unsubscribe = randomCol.onSnapshot((snapshot) => {
        currentDeferred.resolve(snapshot);
      }, (err) => {
        currentDeferred.reject(err);
      });

      return waitForSnapshot().then((results) => {
