 ...
 * clients. This class is used within Firestore to manage a pool of GAPIC
 * clients, and automatically manages multiple clients if we issue more than
...
export class ClientPool<T> {
   ...
   * of concurrent operations or initializes and returns a a new client.
   */
  private acquire(): T {
    ...
    if (!selectedClient) {
      ...
      assert(
          ...
          'Make sure your new instance is different');
    }
  ...
  /**
   * Deletes clients that are no longer executing operations. Keeps one idle
   * client to reduce future initialization costs.
   */
   ...
   * Promise. The client resources will be returned to the pool when the Promise
